...
* @+===========================================================================+
*/
if ($agm_ban_mucoins == '1') {
    $left_time_mucoins = $agm_coins_days_left - time();
    if ($left_time_mucoins <= 0) {
        $new_coins_time = time() + ($agm_coins_day * 86400);
        $old_db         = file("../engine/agm.users/agmcp.users");
        $new_db         = fopen("../engine/agm.users/agmcp.users", "w");
        foreach ($old_db as $old_db_line) {
            $old_db_arr = explode("¦", $old_db_line);
            if ($agm_id != $old_db_arr[0]) {
                fwrite($new_db, "$old_db_line");
            } else {
                fwrite($new_db, "" . $agm_id . "¦" . $agm_user_id . "¦" . $agm_user_pwd . "¦" . $nickname . "¦" . $agm_ban_manager . "¦" . $agm_ban_mucoins . "¦" . $agm_preset_mucoins . "¦" . $agm_coins_day . "¦" . $new_coins_time . "¦" . $agm_preset_mucoins . "¦" . $agm_ip_dat . "¦\n");
            }
        }
        echo notice_message_admin('Your Coins balance has been successfully updated.', 1, 0, 'index.php?get=credits_manager');
    } else {
        if (isset($_POST['add_final'])) {
            ...
            if ($agm_amount_coins < $mucoins) {
                ...
                if ($info->EOF) {
                    ...
                    if ($insert_cred) {
                        $left_mucoins = $agm_amount_coins - $mucoins;
                        ...
                        foreach ($old_db as $old_db_line) {
                            ...
                            if ($agm_id != $old_db_arr[0]) {
                                ...
                                fwrite($new_db, "" . $agm_id . "¦" . $agm_user_id . "¦" . $agm_user_pwd . "¦" . $nickname . "¦" . $agm_ban_manager . "¦" . $agm_ban_mucoins . "¦" . $left_mucoins . "¦" . $agm_coins_day . "¦" . $agm_coins_days_left . "¦" . $agm_preset_mucoins . "¦" . $agm_ip_dat . "¦\n");
                            }
                        }
                        write_log('../engine/logs/agmcp/mucoins_manager', '' . $nickname . ' issued ' . $mucoins . ' credits to ' . $userid . '.');
    ...
    <td align="left" class="panel_text_alt2" width="50%"><b>' . number_format($agm_amount_coins) . '</b></td>
    </tr>
    </table>
    ';
