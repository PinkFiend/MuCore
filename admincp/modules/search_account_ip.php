...
';
    ...
    if (isset($_POST['ip'])) {
        echo '<input type="text" value="' . $_POST['ip'] . '" name="ip">';
    } else {
        echo '<input type="text" value="' . $_SESSION['search_ip'] . '" name="ip">';
    }
...
<td align="left" class="panel_title_sub2" width="50">Controls</td>
            ...
            $user  = $core_db2->Execute("Select top 100 memb___id,ip from memb_stat where ip like ?", array(
                '%' . $userid . '%'
            ));
            $count = 0;
            while (!$user->EOF) {
